            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (mRecordFinishListener != null) {
                        ...
                        if (System.currentTimeMillis() - mDownTime < 2000) {
                            if (mActivity != null) {
                                Toast.makeText(mActivity, "录音时间过短", Toast.LENGTH_SHORT).show();
                            }
                            mRecordFinishListener.onCancel(voiceFile);
                        } else if (distance > 80) {//滑动超过80则提示取消
                            mRecordFinishListener.onCancel(voiceFile);
                        } else {
                            mRecordFinishListener.onFinish(voiceFile, System.currentTimeMillis() - mDownTime);
                        }
                    }
                    break;
